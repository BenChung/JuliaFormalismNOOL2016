...
describes that the method \m{m}{\ldots, \oftype{a}{A}, \ldots} is inside the set described by the type \textit{A} ($\usable{A}$). In \jolt,
this means the method \textit{m} is syntactically defined for \textit{A}. The symbol $\Subset$ denotes a method being enclosed inside a type. 
