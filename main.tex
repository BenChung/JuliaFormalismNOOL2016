...
In this manner, and if we ignore \cum{C}{3} briefly, the programmer has 
...
and lacks a \xt{a}. As a result, line 15 produced the listed error, despite 
...
The concept of creating interfaces for multi-method dispatching is not novel, features such 
as Haskell's type classes \textcolor{red}{[cite]} and Rust's trait systems \textcolor{red}{[cite]} behaves similarly.
In Rust, it is not possible to create orphans (akin to concrete types defined outside the class
whereby it's associated abstract type is defined), however, this is possible in Julia. 
In the case of \jolt, any orphans follow the dynamic behavior of Julia, therefore no static guarantee would
